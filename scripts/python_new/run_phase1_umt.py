...
def objective(exp_name, model_name, dataset_name, lr, wd, phase1):
    ...
    GROUP_NAME = f'{type_names["dataset"]}, {type_names["model"]}, {type_names["optim"]}, epochs={phase1}_overlap={OVERLAP}_lr={lr}_wd={wd}_lambda={LR_LAMBDA}'
...
if __name__ == "__main__":
    ...
    logging.info(f'Script started model s-{conf.model_name} on dataset s-{conf.dataset_name} with lr={conf.lr}, wd={conf.wd}, phase1={conf.phase1}.')
    
    objective('phase1', conf.model_name, conf.dataset_name, conf.lr, conf.wd, conf.phase1)

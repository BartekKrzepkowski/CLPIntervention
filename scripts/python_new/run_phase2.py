...
def objective(exp_name, model_name, dataset_name, lr, wd, phase1, phase2, model_checkpoint):
    ...
    T_max = batches_per_epoch * phase2
    ...
    quick_name = f'trained with phase1={phase1}'
    GROUP_NAME = f'{type_names["dataset"]}, {type_names["model"]}, {type_names["optim"]}, epochs={phase2}_overlap={OVERLAP}_lr={lr}_wd={wd}_lambda={LR_LAMBDA}'
    ...
    config.exp_starts_at_epoch = phase1
    config.exp_ends_at_epoch = phase2 + phase1
...
if __name__ == "__main__":
    ...
    logging.info(f'Script started model s-{conf.model_name} on dataset s-{conf.dataset_name} with lr={conf.lr}, wd={conf.wd}, phase1={phase1}, phase2={conf.phase2}.')
    
    objective('phase2', conf.model_name, conf.dataset_name, conf.lr, conf.wd, conf.phase1, conf.phase2, conf.model_checkpoint)

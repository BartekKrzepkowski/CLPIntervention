...
def objective(exp_name, model_name, dataset_name, lr, wd, phase1, phase2, phase3, phase4, model_checkpoint):
    ...
    GROUP_NAME = f'{type_names["dataset"]}, {type_names["model"]}, {type_names["optim"]}, epochs={phase4}_overlap={OVERLAP}_lr={lr}_wd={wd}_lambda={LR_LAMBDA}'
    ...
    config.exp_starts_at_epoch = phase1 + phase2 + phase3
    config.exp_ends_at_epoch = phase4 + config.exp_starts_at_epoch
...
if __name__ == "__main__":
    ...
    objective('phase4', conf.model_name, conf.dataset_name, conf.lr, conf.wd, conf.phase1, conf.phase2, conf.phase3, conf.phase4, conf.model_checkpoint)

...
def objective(exp_name, model_checkpoint, phase4, lr, wd, phase1, phase2, phase3):
    ...
    T_max = batches_per_epoch * phase4
    ...
    quick_name = f'trained with phase1={phase1}, phase2={phase2}, phase3={phase3}'
    GROUP_NAME = f'{type_names["model"]}, {type_names["dataset"]}, {type_names["optim"]}, overlap={OVERLAP}_epochs={phase4}_lr={lr}_wd={wd}_lambda={LR_LAMBDA}'
    ...
    config.exp_ends_at_epoch = phase4
    ...
    logging.info(f'The dataset has {len(loaders["train"].dataset)} train samples and {len(loaders["test_proper"].dataset)} test samples and {num_classes} classes and every epoch has {batches_per_epoch} batches.')
...
if __name__ == "__main__":
    model_checkpoint = str(sys.argv[1])
    phase4 = int(sys.argv[2])
    lr = float(sys.argv[3])
    wd = float(sys.argv[4])
    phase1 = int(sys.argv[5])
    phase2 = int(sys.argv[6])
    phase3 = int(sys.argv[7])
    ...
    logging.info(f'Script started with phase4={phase4}, lr={lr}, wd={wd}, phase1={phase1}, phase2={phase2}, phase3={phase3}')
    logging.info(f'Model checkpoint path: {model_checkpoint}')
    
    objective('phase4', model_checkpoint, phase4, lr, wd, phase1, phase2, phase3)
